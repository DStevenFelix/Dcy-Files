...
![image-20230608135114913](https://techla-img.oss-cn-hangzhou.aliyuncs.com/CODE/WEB/image-20230608135114913.png)
...
- 喂狗程序的**作用：**
  - **重置或刷新看门狗定时器，以防止看门狗定时器超时**
  - 当系统正常运行时，**喂狗程序会定期执行，向看门狗定时器发送特定的信号，告诉看门狗定时器系统仍在正常工作**
  - 如果系统发生故障或死锁，导致喂狗程序无法执行或超时，看门狗定时器将超时触发，触发相应的动作（如系统复位）以恢复系统的正常状态
- 实现方法：
  - 在硬件层面，可以通过写入特定的寄存器或执行特定的指令来喂狗
  - 在软件层面，可以调用特定的API函数或使用定时器来定期触发喂狗操作
- 喂狗程序的编写需要仔细考虑系统的实际需求和稳定性要求：
  - 喂狗的时间间隔应适当，不能过长或过短，以免误判系统状态
  - 喂狗的执行时间应尽量保持稳定，避免由于执行时间不确定性导致喂狗间隔变化，影响系统的稳定性

#### 注意事项

- 看门狗定时器的中断与复位
  - 中断：
    - **看门狗定时器作为普通定时器应用**，向微处理器**发内部中断请求**
  - 复位：
    - **让微处理器复位，相当于重新启动所有程序**

- **WTDAT存放WDT的计数初值，相当于普通定时器的TCNTBn**
- **WTCNT是WDT的减1计数器，相当于普通定时器的TCNTn**
  - **(程序不可直接访问)**
- 不同于普通定时器，**WTDAT的值再WDT初始使能时，不能自动装载到WTCNT中，所以必须要给WTCNT设定一个初始值**
...
- Shell（Shell解释器）：Shell是一种命令行解释器，它是用户与操作系统内核之间的接口。它接收用户的命令，并将其传递给操作系统执行。在Linux中，常见的Shell包括Bash（Bourne Again SHell）、Zsh（Z Shell）、Ksh（Korn Shell）等。Shell提供了命令解释、脚本编写、变量定义、条件判断等功能，使用户能够与操作系统进行交互。

- Terminal：Terminal是指用户与计算机系统进行交互的终端界面。它是一个图形化的窗口，用于显示Shell的输出结果并接收用户的输入。在Linux系统中，Terminal通常是一个终端仿真器，比如GNOME Terminal、Konsole、Xfce Terminal等。通过Terminal，用户可以打开一个或多个Shell会话，执行命令、查看输出结果，并与Shell进行交互。
